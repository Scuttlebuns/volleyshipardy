...
bool jepHalfRound(double timeToBeat, chrono::duration<double> &elapsed, vector<Question> &question_db) {
	auto start = chrono::high_resolution_clock::now();
	if (!random_question(question_db)) return false;
	else {
...
void jepFullRound(Player &Player1, Player &Player2, vector<Question> &question_db) {
	...
	Question question = question_db.at(index);

	bool player1 = jepHalfRound(initialTimeToBeat, player1Time, question_db);
	...
	bool player2 = jepHalfRound(initialTimeToBeat, player2Time, question_db);
